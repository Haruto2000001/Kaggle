...
from debug import show_map, show_energy_field, show_exploration_map, get_relicnodes, get_rewardnodes
...
from myutils import is_target_in_sap_range, pos_abs2rel
...
class Agent:
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        print2file(f"start step: {match_step}    match num: {match_number}   (step: {step}) \n")
        print2file("########Global Parameters#########")
        print2file(f"UNIT_MOVE_COST: {Global.UNIT_MOVE_COST}")
        print2file(f"UNIT_SAP_COST: {Global.UNIT_SAP_COST}")
        print2file(f"UNIT_SAP_RANGE: {Global.UNIT_SAP_RANGE}")
        print2file(f"UNIT_SENSOR_RANGE: {Global.UNIT_SENSOR_RANGE}")
        print2file(f"ALL_RELICS_FOUND: {Global.ALL_RELICS_FOUND}")
        print2file(f"ALL_REWARDS_FOUND: {Global.ALL_REWARDS_FOUND}")
        print2file(f"RELIC_REWARD_RANGE: {Global.RELIC_REWARD_RANGE}")
        print2file("##################################")
        ...
        for ship in self.fleet:
            print2file(f"ship:{ship}, task:{ship.task}, target:{ship.target.coordinates if ship.target is not None else None}, action:{ship.action}")
        print2file("##################################\n")

        print2file("###############Opponents Ship###############")
        for oppship in self.opp_fleet:
            print2file(f"ship:{oppship}")
        print2file("############################################\n")
            #print(f"obs.unit_mask: {obs['units_mask']}", file=logfile)
            #print(f"obs.units.position: {obs['units']['position']}", file=logfile)
    ...
    def create_actions_array(self):
        ...
        for i, ship in enumerate(ships):
            if ship.action is not None:
                if ship.action == 5:
                    if (ship.energy-Global.UNIT_SAP_COST)>Global.UNIT_MOVE_COST+5:
                        actions[i] = ship.action, *pos_abs2rel(ship.node.coordinates, ship.target.coordinates)
                    else:
                        actions[i] = 0, 0, 0
                else:
    ...
    def find_rewards(self):
        ...
        print2file(f"unexplored_relic: {unexplored_relics}")
        relic_node_to_ship = {}
        for ship in self.fleet:
            if ship.task == "find_rewards":
        ...
        for relic in unexplored_relics:
            ...
            # relic_node_to_shipに一番近いship

        
        print2file(f"relic_node_to_ship: {relic_node_to_ship}")

        def set_task(ship, relic_node, can_pause):
            targets = []

            # レリックの周りでリワード未探索のノードをtargetsに入れる
            ...
            # もし今いるところがtargetだったら自分の周りでまだ未探索のノードに変える
            ...
            print2file(f"ship: {ship} target: {target}")
            if not target:
                return
            ...
            if actions and ship.energy >= energy:
                ...
                print2file(f"ship.task: {ship.task} ship.target: {ship.target} ship.action: {ship.action}")
                return True
    ...
    def find_rewards_multi_ships(self):
        # 片方だけポイントとかだとわからない？
        if Global.ALL_REWARDS_FOUND:
            for ship in self.fleet:
                if ship.task == "find_rewards":
        ...
        for relic in unexplored_relics:
            if relic not in relic_node_to_ship:
                ...
                for ship in self.fleet:
                    ...
                    distance = manhattan_distance(ship.coordinates, relic.coordinates)
                    ship_distances.append([distance, ship])

                # 近い順にソート
                ship_distances.sort(key=lambda x: x[0]) # distanceが同じ場合のバグを回避
                # shipだけ取り出す
                sorted_ships = [ship for _, ship in ship_distances]


                # 上位何番目までをfind_rewardにするか
                #sorted_ships = sorted_ships[:4] # 選べる
            
                
                print2file(f"sorted_ships: {sorted_ships}")

                # 辞書に保存
                if not isinstance(sorted_ships, list):
                    relic_node_to_ship[relic] = [sorted_ships]
                else:
                    relic_node_to_ship[relic] = sorted_ships
        ...
        def set_task_random(ship, relic_node, can_pause):
            targets = []
        ...
        print2file(f"relic_node_to_ship.items(): {relic_node_to_ship.items()}")
                    
        for n, ss in relic_node_to_ship.items():
            
            print2file(f"ss: {ss}")
            for s in ss:
                for _ in range(10):  # 再試行用ループ
                    if set_task_random(s, n, can_pause):
                        if s.target in booked_targets:
                            # すでに予約されたターゲットならやり直し
                            s.target = None  # リセット
                            s.task = None  # タスクもリセット
                            continue  # 再試行
                        else:
                            booked_targets.append(s.target)  # 予約リストに追加
                            break  # 成功したのでループを抜ける
                    else:
                        if s.task == "find_rewards":
                            s.task = None
                            s.target = None
                        break  # 失敗した場合は再試行せず終了
    ...
    def sap_a(self):
        for ship in self.fleet:
            if ship.task=="harvest" and \
                ship.target is not None and\
                      not is_team_sector(self.team_id, *ship.target.coordinates) and \
                        is_target_in_sap_range(Global.UNIT_SAP_RANGE, ship.node.coordinates, ship.target.coordinates):
                
                ship.action = ActionType.sap

                
                print2file(f"{ship} can sap at {ship.target.coordinates}")
    ...
    """my codes"""
    def get_relicnodes(self):
        return get_relicnodes(self.team_id, self.space)
    
    def get_rewardsnode(self):
        return get_rewardnodes(self.team_id, self.space)